...
- **Data hooks**: one hook module per feature domain in `hooks/` (`use-routine`, `use-consistency`, `use-doubts`, `use-notes`, `use-resources`, `use-flashcards`, `use-practice`, `use-curiosity`, `use-reflection`, `use-onboarding-chat`, `use-profile-analysis`), each wrapping Supabase queries/mutations in React Query. Domain logic that isn't a direct DB call lives in `lib/` (`streak.ts` for streak bump logic, `flashcard-scheduler.ts` for spaced-interval staging, `doubt-helper.ts` for canned mentor answers/rephrase hints, `curiosity-prompts.ts`, `practice-partners.ts`, `notifications.ts`, `trigger-analysis.ts` for firing Gemini re-analysis after activity — see "AI personalization" below).
- **Most "smart" features are still intentionally simplified**, not real ML/scheduling — an explicit scope tradeoff, not a placeholder to "finish later": flashcard spacing is stage-based fixed intervals (`lib/flashcard-scheduler.ts`), practice-partner matching is a static pool with recent-partner avoidance (`lib/practice-partners.ts`), and doubt answers are keyword-matched canned responses (`lib/doubt-helper.ts`). Onboarding and the personalized dashboard are the exception — those are genuinely Gemini-driven, see "AI personalization" below.
